...
const serviceSchema = new mongoose.Schema({
  ...
  subCategory: [{ code: String, label: String }],
});
...
app.get('/tasks/user/:userId', authenticate, async (req, res) => {
...
app.get('/api/jobs', authenticate, async (req, res) => {
  ...
  app.post('/set-profile', authenticate, [
    // User basic information validations
    body('username').trim().escape().isLength({ min: 1 }).withMessage('Username is required'),
    body('title').trim().escape(),
    body('description').trim().escape(),

    // Skills validation
    body('skills.*.code').trim().escape(),
    body('skills.*.label').trim().escape(),

    // Education validation
    body('education.*.school').trim().escape(),
    body('education.*.degree').trim().escape(),
    body('education.*.fieldOfStudy').trim().escape(),
    body('education.*.startMonth').trim().escape(),
    body('education.*.startYear').isInt({ min: 1900, max: new Date().getFullYear() }),
    body('education.*.endMonth').trim().escape(),
    body('education.*.endYear').isInt({ min: 1900, max: new Date().getFullYear() + 10 }),
    body('education.*.currentlyStudyingHere').isBoolean(), // Added validation for currentlyStudyingHere

    // Experience validation
    body('experience.*.title').trim().escape(),
    body('experience.*.companyName').trim().escape(),
    body('experience.*.location').trim().escape(),
    body('experience.*.locationType').trim().escape(),
    body('experience.*.employementType').trim().escape(),
    body('experience.*.currentlyWorkingHere').isBoolean(),
    body('experience.*.startDate').trim().escape(),
    body('experience.*.startYear').isInt({ min: 1900, max: new Date().getFullYear() }),
    body('experience.*.endDate').trim().escape(),
    body('experience.*.endYear').isInt({ min: 1900, max: new Date().getFullYear() }),

    // Other details validation
    body('otherDetails.timeZone.region').trim().escape(),
    body('otherDetails.timeZone.timeZone').trim().escape(),
    body('otherDetails.payment').trim().escape(),
    body('otherDetails.language').trim().escape(),
    ...
    try {
        ...
        user.skills = req.body.skills; // Assign new skills array
...
app.put('/update-profile/:userId', authenticate, async (req, res) => {
  const { userId } = req.params;
...
app.patch('/edit-profile/:userId', authenticate, async (req, res) => {
  const { userId } = req.params;
